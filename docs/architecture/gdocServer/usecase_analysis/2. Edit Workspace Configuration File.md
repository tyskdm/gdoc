...
sequenceDiagram
  ...
  IDE -) +LS: textDocument/didSave /<br/>didChangeWatchedFiles (config)
    LS ->> +ODB: CONFIG_SAVE (workspace root + config location)
      Note over ODB: ODB reads & parses new configuration<br/>(D-019: ODB owns the config)<br/>Define Project · Packages · Documents<br/>Compare with existing state
