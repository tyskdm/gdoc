...
> **Status:** Draft for user review (Phase 1b — not yet approved).
...
| D-011 · retention window (fetch-once + TTL + expiry notify) | OK (provisional) | API-002, §5, §5.2, §7 | TTL threshold deferred (D-007 / R-007-2); D-011 not yet in `README.md` | log D-011 in `README.md` §7 at approval (P-7) |
| P-8 · Frontend identity (implicit via API object) + one-live-handler replace + cross-Frontend isolation | OK (resolved) | API-003, API-004, §4.2/§4.4, §5 (`E_NOT_FOUND`) | no new ADR needed (grounded in ADR-001/003/008); no `README.md` entry required | record at Phase 1b approval (P-8) |
...
*End of contract. Status: Draft for user review (Phase 1b).*
