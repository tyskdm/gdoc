...
> **`DOCUMENT_SYNC` payload convention (content requirement; D-016/D-017, clarified 2026-09-25 — D-021).** The `payload` of `DOCUMENT_SYNC` requires the **`action`** discriminator (`"open" | "change" | "close"`, D-016) and, for `action:"open"` and `action:"change"` (an **open** document), the **`content`** — the **full updated buffer**, since the ODB/Builder build from it (§4.3 C1 obligation; the ODB cannot reconstruct it from previous submissions). For `action:"close"` **`content` is optional (absent)** — the transition carries no content. This makes D-017's `DOCUMENT_SYNC` minimum-fields entry (`action` + `content?`) unambiguous: the `?` applies **only to the `close` branch**. (UC-003 analysis finding, user-approved 2026-09-25.)
