...
| Q-002 | Document freshness / identity method | answered | **D-004** (LSP `version` = monotonic revision counter) |
...
| D-004 | 2026-09-11 | Document identity/freshness = **LSP `version`** (per-document monotonic revision counter) used as the dedup key / freshness / Datastore identity key; "assign a version once stable" is an optional upper layer (not needed in v1) | Dedup/freshness needs a per-change monotonic counter, which LSP already provides as `version`; a labeled version only complicates the key (resolves Q-002). | ✅ |
...
- **Procedure:**
  ...
  5. **Document identity / freshness** (Q-002 → **D-004**): LSP `version` as the key; reflected in
