...
| **Design review records** | `process/reviews/` | Per-round cross-document review records (NC-*/OM-*/FU-*/RV-*); index + shared conventions in `process/reviews/README.md`; procedure in `process/reviews/Design Review Procedure.md` (repo-resident — the `.agents/` setup is not yet stable) | **active** (findings ongoing) |
