...
| D-004 | 2026-09-11 | Document identity/freshness = **`version_id` = tuple `(last_save_timestamp, open_revision)`**: `last_save_timestamp` = last on-disk save mtime; `open_revision` = LSP `didChange` version for **open** files, **0** for **non-open** files; compare mtime-then-revision; **C1 forwards the raw sync facts it observes** (open: `didChange` version; last-save mtime when it has one) and **C2 (ODB) owns & composes** the `version_id` (non-open mtime obtained by C2 from disk / `didChangeWatchedFiles`); C2 uses it as the dedup-key version (TJ-005) + Datastore freshness invariant (TJ-018); content: open → client **buffer**, non-open → builder reads **disk** by path | Dedup/freshness need a per-change, comparable key for **both open and non-open** files: LSP `version` covers open buffers (unsaved edits), `didChangeWatchedFiles`+mtime covers disk files; one tuple unifies them (resolves Q-002). | ✅ |
...
- **Procedure:**
  ...
  5. **Document identity / freshness** (Q-002 → **D-004**): `version_id` = (last-save mtime, open_revision) as the key (open=buffer, non-open=disk; C1 forwards the raw sync facts, **C2 owns & composes the key**); reflected in
